...
void moveBeforeFirstUser(Instruction *I, Loop *L, const DominatorTree &DT) {
  Instruction *FirstUser = nullptr;
  for (auto *LUser : I->users()) {
    auto *LUserI = cast<Instruction>(LUser);
    if (isa<PHINode>(LUserI) || !L->contains(LUserI))
      continue;
    if (!FirstUser) {
      FirstUser = LUserI;
      continue;
    }
    if (!DT.dominates(FirstUser, LUserI)) {
      assert(LUserI == FirstUser || DT.dominates(LUserI, FirstUser));
      FirstUser = LUserI;
    }
  }
  if (!FirstUser)
    return;
  I->moveBefore(FirstUser);
}

Value *copyTermOp(Value *V, BasicBlock *OldBB, BasicBlock *NewBB) {
  auto *I = dyn_cast<Instruction>(V);
  if (!I || I->getParent() != OldBB)
    return V;
  SmallVector<std::pair<Value *, Value *>, 8> Ops;
  for (auto &Op : I->operands())
    Ops.push_back(std::make_pair(Op.get(), copyTermOp(Op, OldBB, NewBB)));

  auto *CopyI = I->clone();
  CopyI->insertBefore(NewBB->getTerminator());
  for (const auto &P : Ops)
    CopyI->replaceUsesOfWith(P.first, P.second);
  return CopyI;
}


void createExpressionTree(BasicBlock &BB, Loop *L, const DominatorTree &DT) {
    SmallVector<Instruction *, 8> Insts;
    SmallPtrSet<Instruction *, 8> OutsideUsers;
    ...
    for (auto It = Insts.rbegin(), End = Insts.rend(); It != End; It++) {
      ...
      for (auto *User : I->users()) {
        ...
        if (!L->contains(UserI))
          OutsideUsers.insert(UserI);
      }
      Instruction *OutsideCopy = nullptr;
      for (auto *UserI : OutsideUsers) {
        if (!OutsideCopy) {
          OutsideCopy = I->clone();
          OutsideCopy->setName(I->getName() + "_1u");
          OutsideCopy->insertBefore(&*L->getExitBlock()->getFirstInsertionPt());
        }
        assert(!isa<PHINode>(UserI));
        UserI->replaceUsesOfWith(I, OutsideCopy);
        //if (auto *LU = dyn_cast<LoadInst>(OutsideCopy))
          //LU->setVolatile(true);
      }
      moveBeforeFirstUser(I, L, DT);
    }
}
void moveBeforeFirstUserRec(Instruction *I, Loop *L, const DominatorTree &DT,
  ...
  for (auto &Op : I->operands()) {
    ...
    if (!OpI || !L->contains(OpI) || isa<PHINode>(OpI))
      continue;
    moveBeforeFirstUserRec(OpI, L, DT, Visited);
  }
}
...
void InnerLoopVectorizer::emitMinimumIterationCountCheck(Loop *L,
  ...
  //MDNode *Node = MDB.createBranchWeights(1, 2000);
...
void InnerLoopVectorizer::createVectorizedLoopSkeleton() {
  ...
  for (auto &InductionEntry : *List) {
    ...
    if (BlockIdx == (unsigned)-1) {
      OrigPhi->getFunction()->dump();
      errs() << "ORIGPHI: " << *OrigPhi<<"\n";
      errs() << "ScalarPI: " << *ScalarPH<<"\n";
    }
  ...
  for (unsigned i = 0, e = VFs.size(); i < e; ++i) {
    ...
    if (VFs[i] == 1)
      ...
      for (auto Inst : LoopInvariants) {
        DEBUG(errs() << "LOOP INVARIANT INST: " << *Inst << " [RU: "
                     << GetRegUsage(Inst->getType(), VFs[i]) << "]\n");
...
static void orderPHI(PHINode *PHI, Loop *L, BasicBlock *LatchBB,
                     BasicBlock *Header) {
  auto *InVal = PHI->getIncomingValueForBlock(LatchBB);
  auto *OpPHI = dyn_cast<PHINode>(InVal);

  if (!OpPHI) {
    PHI->moveBefore(&*Header->begin());
    if (auto *InValInst = dyn_cast<Instruction>(InVal))
      movePrior(InValInst, L);
  } else {
    orderPHI(OpPHI, L, LatchBB, Header);
    PHI->moveBefore(OpPHI->getNextNode());
  }
}

static void orderPHIs(Loop *L, const DominatorTree &DT) {
  ...
  Instruction *FIP = BitCastInst::CreateBitOrPointerCast(
      UndefValue::get(Type::getInt8PtrTy(Ctx)), Type::getInt8PtrTy(Ctx));
  FIP->insertBefore(HeaderBB->getFirstNonPHIOrDbgOrLifetime());
  ...
  std::function<void(PHINode *)> OrderPHIOp = [&](PHINode *PHI) {
    ...
    if (auto *LatchOpPHI = dyn_cast<PHINode>(LatchOpI)) {
      if (LatchOpPHI->getParent() == HeaderBB) {
...
static void duplicate(Loop *L, Value *V) {
  auto *I = dyn_cast<Instruction>(V);
  if (!I || isa<PHINode>(I))
    return;
  ...
  DEBUG(errs() << "Dupl: " << *I << " #ILO: " << InLoopOps.size()
               << " #US: " << I->getNumUses() << "\n");
  ...
  if (I->getNumUses() > 1) {
    ...
    for (auto *User : I->users()) {
      ...
      if (!First) {
        auto *CopyI = I->clone();
        CopyI->insertBefore(UserI);
        DEBUG(errs() << "Clone " << *I << " : " << *CopyI << "\n");
        UserI->replaceUsesOfWith(I, CopyI);
      }
      First = false;
    }
  }
  if (I->getNumUses() == 1) {
    I->moveBefore(I->user_back());
    DEBUG(errs() << "Move: " << *I << " with " << I->getNumUses() << " users\n");
  } else {
    DEBUG(errs() << "Inst: " << *I << " has " << I->getNumUses() << " users\n");
  }
}

static void removeOperands(Instruction *I,
                           DenseMap<Instruction *, unsigned> &LeftUsesMap) {
  for (auto &Op : I->operands())
    if (auto *OpI = dyn_cast<Instruction>(Op)) {
      if (LeftUsesMap.count(OpI)) {
        unsigned &OpILeftUses = LeftUsesMap[OpI];
        assert(OpILeftUses > 0);
        OpILeftUses--;
        if (OpILeftUses == 0)
          LeftUsesMap.erase(OpI);
      }
    }
}

static unsigned computeNumOfRegsNeeded(BasicBlock *BB, BasicBlock *PreHeader) {
  DenseMap<Instruction *, unsigned> LeftUsesMap;
  unsigned MaxUsage = 0;
  SmallPtrSet<Value *, 8> OutsideOps;
  for (Instruction &I : *BB) {
    SmallPtrSet<Instruction *, 8> Users;
    for (auto &Op : I.operands())
      if (auto *OpI = dyn_cast<Instruction>(Op)) {
        if (OpI->getParent() != BB)
          OutsideOps.insert(Op);
      }
    for (auto &Use : I.uses())
      if (auto *UseI =  dyn_cast<Instruction>(Use))
        Users.insert(UseI);
    if (Users.size())
      LeftUsesMap[&I] = Users.size();
    removeOperands(&I, LeftUsesMap);
    MaxUsage = std::max(LeftUsesMap.size(), MaxUsage);
    errs() << "RM/RC " << MaxUsage << "/" << LeftUsesMap.size() << " for " << I
           << "\t";
    for (auto It : LeftUsesMap)
      errs() << "[" << It.first->getName() << ":"<< It.second << "]";
    errs() << "\n";
  }
  for (auto *OutsideV : OutsideOps)
    errs() << "Outside: " << *OutsideV << "\n";
  errs() << "Total outside:  " << OutsideOps.size() << "\n";
  return MaxUsage;
}

static unsigned computeNumOfRegsNeeded(Value *V) {
  ...
  if (I->getNumOperands() == 1)
    return std::max((unsigned)1,
                    computeNumOfRegsNeeded(I->getOperand(0)));
  if (I->getNumOperands() == 2) {
    auto NeedOp0 = computeNumOfRegsNeeded(I->getOperand(0));
    auto NeedOp1 = computeNumOfRegsNeeded(I->getOperand(1));
    if (NeedOp0 == NeedOp1)
      return 1 + NeedOp0;
    return std::max(std::max((unsigned)1, NeedOp0), NeedOp1);
  }
  DenseSet<unsigned> OpNeeds;
  for (auto &Op : I->operands())
    OpNeeds.insert(computeNumOfRegsNeeded(Op));
  if (OpNeeds.size() == 1)
    return *OpNeeds.begin() + 1;
  unsigned MaxOpNeeds = 1;
  std::for_each(OpNeeds.begin(), OpNeeds.end(), [&](unsigned OpNeed) {
    MaxOpNeeds = std::max(MaxOpNeeds, OpNeed);
  });
  return MaxOpNeeds;
}
...
bool LoopVectorizePass::processLoop(Loop *L) {
  ...
  DEBUG(for (auto *BB : L->blocks()) errs() << *BB << "\n";);
  ...
  for (auto *BB : L->blocks()) {
    Size++;
    AnyPolly |= BB->getName().contains("polly");
    for (auto &I : *BB)
      if (auto *S = dyn_cast<StoreInst>(&I)) {
        OneStore = Store == nullptr;
        Store = S;
      }
  }
  DEBUG({
    errs() << "One Store: " << OneStore << "\n";
    if (Store && OneStore)
      errs() << "Store: " << *Store << "\n";
  });
  if (AnyPolly && Size == 1) {
    ////duplicate(L, Store->getValueOperand());
    orderPHIs(L, *DT);
    ...
    ////unsigned MaxUsage =
        ////computeNumOfRegsNeeded(Store->getParent(), L->getLoopPredecessor());
    ////errs() << "Register cost 3: " << MaxUsage << "\n";
  }

  DEBUG(dbgs() << "\nLV: Checking a loop in \""
               << L->getHeader()->getParent()->getName() << "\" from "
               << DebugLocStr << "\n");
  DEBUG(errs() << "Annotated Parallel: " << L->isAnnotatedParallel()
               << ", Simplified form: " << L->isLoopSimplifyForm() << "\n");
  ...
  if (VecLoop) {
    //auto MaxUsage = computeNumOfRegsNeeded(VecLoop->getHeader(),
                                      //VecLoop->getLoopPredecessor());
    //errs() << "Register cost sorted vec: " << MaxUsage << "\n";

    SmallVector<StoreInst *, 8> Stores;
    for (auto *BB : VecLoop->blocks())
      for (auto &I : *BB)
        if (auto *S = dyn_cast<StoreInst>(&I))
          Stores.push_back(S);
    ...
    //for (auto *S : Stores)
      //moveBeforeFirstUserRec(S, VecLoop, *DT, Visited);
  ...
  if (VecLoop) {
    ...
    auto *LatchTI = VecLoop->getLoopLatch()->getTerminator();
    MDBuilder MDB(LatchTI->getContext());
    MDNode *Node;
    if (LatchTI->getSuccessor(0) == VecLoop->getHeader())
      Node = MDB.createBranchWeights(500, 1);
    else
      Node = MDB.createBranchWeights(1, 500);
    LatchTI->setMetadata(LLVMContext::MD_prof, Node);
    ...
    for (auto *BB : VecLoop->blocks()) {
      for (auto &I : *BB)
        Insts.push_back(&I);
      for (auto *I : Insts) {
        if (I->getNumUses() == 0 && !isa<TerminatorInst>(I) &&
    ...
    // auto *Id = MDNode::get(Ctx, {UCMetadata});
    // auto *IdMD = VecLoop->getLoopID();
    // assert(IdMD);
    // IdMD = MDNode::concatenate(IdMD, Id);
    // IdMD->replaceOperandWith(0, IdMD);
    // VecLoop->setLoopID(IdMD);

    unsigned MaxUsage = computeNumOfRegsNeeded(VecLoop->getHeader(),
                                               VecLoop->getLoopPredecessor());
    errs() << "Register cost vec: " << MaxUsage << "\n";
    // if (MaxUsage > 16) {
    // createExpressionTree(*Store->getParent(), VecLoop, *DT);
    SmallPtrSet<Instruction *, 32> Visited;
    for (auto *Store : Stores) {
      //moveBeforeFirstUserRec(Store, VecLoop, *DT, Visited);
      //sortBB(Store, VecLoop, TTI, *DT);
    }
    assert(!verifyFunction(*L->getHeader()->getParent(), &errs()));
    MaxUsage = computeNumOfRegsNeeded(VecLoop->getHeader(),
                                      VecLoop->getLoopPredecessor());
    errs() << "Register cost sorted vec: " << MaxUsage << "\n";
    //}

    auto *ExitBB = VecLoop->getExitBlock();
    auto *ExitingBB = VecLoop->getExitingBlock();
    assert(ExitBB && !isa<PHINode>(ExitBB->begin()));
    assert(ExitingBB);
    auto *ScalarBB = ExitBB->getTerminator()->getSuccessor(0);
    if (!ScalarBB->getName().count("scalar.ph"))
      ScalarBB = ExitBB->getTerminator()->getSuccessor(1);
    if (!ScalarBB->getName().count("scalar.ph"))
      ScalarBB = nullptr;
    bool MayWrite = false;
    for (auto &I : *ExitBB) {
      MayWrite |= I.mayHaveSideEffects();
      if (I.mayHaveSideEffects())
        I.dump();
    }
    if (!MayWrite && ScalarBB) {
      auto *NewExitBB =
          BasicBlock::Create(ExitBB->getContext(), ExitBB->getName() + ".split",
                             ExitBB->getParent(), ExitBB);
      if (auto *LL = LI->getLoopFor(ExitBB))
        LL->addBasicBlockToLoop(NewExitBB, *LI);
      DT->addNewBlock(NewExitBB, ExitingBB);
      DT->changeImmediateDominator(ExitBB, NewExitBB);

      MDBuilder MDB(ExitBB->getContext());
      MDNode *Node;
      if (ExitBB->getTerminator()->getSuccessor(0) == ScalarBB)
        Node = MDB.createBranchWeights(1, 2000);
      else
        Node = MDB.createBranchWeights(2000, 1);
      ExitBB->getTerminator()->setMetadata(LLVMContext::MD_prof, Node);
      ExitingBB->getTerminator()->replaceUsesOfWith(ExitBB, NewExitBB);
      BranchInst::Create(ExitBB, NewExitBB);
      copyTermOp(ExitBB->getTerminator(), ExitBB, NewExitBB);
      NewExitBB->getTerminator()->eraseFromParent();
      NewExitBB->getTerminator()->replaceUsesOfWith(ScalarBB, ExitBB);
      NewExitBB->getTerminator()->setMetadata(LLVMContext::MD_prof, Node);

      // auto *PredTI = VecLoop->getLoopPredecessor()->getTerminator();
      // if (PredTI->getSuccessor(0) == VecLoop->getHeader())
      // Node = MDB.createBranchWeights(500, 1);
      // else
      // Node = MDB.createBranchWeights(1, 500);
      // PredTI->setMetadata(LLVMContext::MD_prof, Node);
    }
    assert(!verifyFunction(*L->getHeader()->getParent(), &errs()));

    for (auto *PHI : PHIs) {
      auto *AI = DemotePHIToStack(PHI);
      for (auto *User : AI->users())
        if (auto *LU = dyn_cast<LoadInst>(User)) {
          // LU->setVolatile(true);
          Instruction *FirstUser = LU->user_back();
          for (auto *LUser : LU->users()) {
            auto *LUserI = cast<Instruction>(LUser);
            if (!DT->dominates(FirstUser, LUserI)) {
              assert(LUserI == FirstUser || DT->dominates(LUserI, FirstUser));
              FirstUser = cast<Instruction>(LUserI);
            }
          }
          LU->moveBefore(FirstUser);
        }
      for (auto *User : AI->users())
        if (auto *SU = dyn_cast<StoreInst>(User)) {
          if (!VecLoop->contains(SU->getParent()))
            continue;
          Instruction *FirstOpI = cast<Instruction>(SU->getValueOperand());
          for (auto *User2 : AI->users())
            if (auto *LU = dyn_cast<LoadInst>(User2)) {
              if (DT->dominates(FirstOpI, LU)) {
                assert(LU == FirstOpI || !DT->dominates(LU, FirstOpI));
                FirstOpI = cast<Instruction>(LU);
              }
            }
          SU->moveBefore(FirstOpI->getNextNode());
        }
    }
    ...
    if (PHIs.size()) {
      auto MaxUsage = computeNumOfRegsNeeded(VecLoop->getHeader(),
                                             VecLoop->getLoopPredecessor());
      errs() << "Register cost after phi demotion: " << MaxUsage << "\n";
    }
    assert(!verifyFunction(*L->getHeader()->getParent(), &errs()));
  }
#endif
