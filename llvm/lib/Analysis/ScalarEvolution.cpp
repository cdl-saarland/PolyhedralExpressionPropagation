...
namespace {
class SCEVUnkownCounter : public SCEVRewriteVisitor<SCEVUnkownCounter> {
public:
  static void
  count(const SCEV *S, ScalarEvolution &SE,
        DenseMap<const SCEVUnknown *, const SCEVConstant *> &CounterMap) {
    SCEVUnkownCounter Rewriter(SE, CounterMap);
    Rewriter.visit(S);
  }
  ...
  SCEVUnkownCounter(
      ScalarEvolution &SE,
      DenseMap<const SCEVUnknown *, const SCEVConstant *> &CounterMap)
      : SCEVRewriteVisitor(SE), SE(SE), CounterMap(CounterMap) {}

  const SCEV *visitUnknown(const SCEVUnknown *Expr) {
    auto *&CC = CounterMap[Expr];
    if (!CC || CC->isOne())
      CC = cast<SCEVConstant>(SE.getOne(Expr->getType()));
    else
      CC = cast<SCEVConstant>(SE.getZero(Expr->getType()));
    return Expr;
  }

  const SCEV *visitMulExpr(const SCEVMulExpr *Expr) {
    DenseMap<const SCEVUnknown *, const SCEVConstant *> OpCounterMap;
    const SCEVConstant *C = cast<SCEVConstant>(SE.getOne(Expr->getType()));
    for (unsigned u = 0; u < Expr->getNumOperands(); u++) {
      auto *Op = Expr->getOperand(u);
      if (isa<SCEVConstant>(Op)) {
        C = cast<SCEVConstant>(SE.getMulExpr(C, Op));
      } else {
        SCEVUnkownCounter::count(Op, SE, OpCounterMap);
      }
    }
    auto CTS = SE.getTypeSizeInBits(C->getType());
    for (auto &It : OpCounterMap) {
      auto *Op = It.second;
      auto OpTS = SE.getTypeSizeInBits(Op->getType());
      auto *OpC = C;
      if (OpTS > CTS)
        OpC = cast<SCEVConstant>(SE.getSignExtendExpr(OpC, Op->getType()));
      else if (OpTS < CTS)
        Op = cast<SCEVConstant>(SE.getSignExtendExpr(Op, OpC->getType()));
      CounterMap[It.first] = cast<SCEVConstant>(SE.getMulExpr(Op, OpC));
    }
    return Expr;
  }
...
static const SCEV *removeConstantFactors(ScalarEvolution &SE, const SCEV *T, const SCEVConstant **F = nullptr);
...
public:
  ...
  static void divide(ScalarEvolution &SE, const SCEV *Numerator,
    ...
    DEBUG(errs() << "DIVIDE: " << *Numerator << " / " << *Denominator << "\n");
    ...
    if (isa<SCEVConstant>(Denominator)) {
      const SCEVConstant *F = nullptr;
      auto *Rem = removeConstantFactors(SE, Numerator, &F);
      if (Rem != Numerator && F) {
        const SCEV *Q, *R;
        divide(SE, F, Denominator, &Q, &R);
        if (R->isZero()) {
          *Quotient = SE.getMulExpr(Q, Rem);
          *Remainder = SE.getZero(Numerator->getType());
          return;
        }
      }
    }
    ...
    *Remainder = D.Remainder;
    ...
    if (D.Quotient->isZero() && D.Remainder == Numerator) {
      DEBUG(errs() << "\tDIVIDE: " << *Numerator << "/" << *Denominator << " failed!\n");

      DenseMap<const SCEVUnknown *, const SCEVConstant *> NumCM, DenCM;
      const SCEV *Factor = nullptr;
      SCEVUnkownCounter::count(Numerator, SE, NumCM);
      SCEVUnkownCounter::count(Denominator, SE, DenCM);
      if (DenCM.empty()) {
        DEBUG(errs() << "\tDenCM empty!\n");
        return;
      }
      if (DenCM.size() != NumCM.size()) {
        DEBUG(errs() << "\tDenCM.size() != NumCM.size()!\n");
        return;
      }

      auto NumTyS = SE.getTypeSizeInBits(Numerator->getType());
      for (auto &It : DenCM) {
        auto *UD = It.first;
        const SCEV *QD = It.second;
        assert(UD && QD);
        DEBUG(errs() << "\tDenCM: " << *UD << " #= " << *QD << " : "
                     << *QD->getType() << "\n");
        const SCEV *QN = NumCM.lookup(UD);
        if (!QN || QD->isZero())
          return;

        DEBUG(errs() << "\tNumCM: " << *UD << " #= " << *QN << " : "
                     << *QN->getType() << "\n");
        DEBUG(errs() << "\tNumerator: " << *Numerator << " : "
                     << *Numerator->getType() << "\n");
        auto QNTyS = SE.getTypeSizeInBits(QN->getType());
        if (QNTyS > NumTyS)
          return;
        if (QNTyS < NumTyS)
          QN = SE.getSignExtendExpr(QN, Numerator->getType());
        auto QDTyS = SE.getTypeSizeInBits(QD->getType());
        if (QDTyS > NumTyS)
          return;
        if (QDTyS < NumTyS)
          QD = SE.getSignExtendExpr(QD, Numerator->getType());

        const SCEV *Fac, *Rem;
        divide(SE, QN, QD, &Fac, &Rem);
        DEBUG(errs() << "\tFac: " << *Fac << " Rem: " << *Rem << "\n");
        if (!Rem->isZero() || Fac->isZero())
          return;
        if (Factor && Factor != Fac)
          return;
        Factor = Fac;
      }
      DEBUG(errs() << "\tFactor: " << *Factor << "\n");
      const SCEV *QNum = SE.getMulExpr(Denominator, Factor);
      const SCEV *RNum = SE.getMinusSCEV(Numerator, QNum);
      *Quotient = Factor;
      *Remainder = RNum;
      DEBUG(errs() << "\tSOLVED: " << *Numerator << "/" << *Denominator << " = "
                   << *Factor << "\n\tRem: " << *RNum << "!\n");
    } else {
      DEBUG(errs() << "DI1: " << *Numerator << "/" << *Denominator << " = "
  ...
  void visitSignExtendExpr(const SCEVSignExtendExpr *Numerator) {
    const SCEV *NewD = nullptr;
    auto *SExtDen = dyn_cast<SCEVSignExtendExpr>(Denominator);
    if (SExtDen)
      NewD = SExtDen->getOperand();
    auto *Ty = Numerator->getOperand()->getType();
    if (auto *SC = dyn_cast<SCEVConstant>(Denominator))
      NewD = SE.getConstant(Ty, SC->getValue()->getSExtValue());
    if (!NewD) {
      bool IsAdd =isa<SCEVAddExpr>(Denominator);
      bool IsMul =isa<SCEVMulExpr>(Denominator);
      auto *SNAryDen = dyn_cast<SCEVNAryExpr>(Denominator);
      if (!SNAryDen || !(IsAdd || IsMul))
        return;
      SmallVector<const SCEV *, 4> Ops;
      for (auto *Op : SNAryDen->operands()) {
        if (isa<SCEVConstant>(Op))
          Ops.push_back(SE.getTruncateOrNoop(Op, Ty));
        else if (auto *OpSE = dyn_cast<SCEVSignExtendExpr>(Op))
          Ops.push_back(OpSE->getOperand());
        else
          return;
      }
      if (IsAdd)
        NewD = SE.getAddExpr(Ops);
      else if (IsMul)
        NewD = SE.getMulExpr(Ops);
      else
        llvm_unreachable("not add & not mul :(");
    }
    assert(NewD);

    const SCEV *Q, *R;
    divide(SE, Numerator->getOperand(), NewD, &Q, &R);
    Quotient = SE.getNoopOrSignExtend(Q, Numerator->getType());
    Remainder = SE.getNoopOrSignExtend(R, Numerator->getType());
  }
  ...
  void visitMulExpr(const SCEVMulExpr *Numerator) {
    ...
    auto *NC = C;
    for (const SCEV *Op : Numerator->operands()) {
      if (auto *SC = dyn_cast<SCEVConstant>(Op))
        C = SE.getMulExpr(C, SC);
      else
        NC = SE.getMulExpr(NC, Op);
    }

    if (isa<SCEVConstant>(Denominator)) {
      const SCEV *Q, *R;
      divide(SE, C, Denominator, &Q, &R);
      Quotient = SE.getMulExpr(Q, NC);
      Remainder = SE.getMulExpr(R, NC);
      return;
    }

    if (Numerator->getNumOperands() == 2)
      C = SE.getOne(Ty);
...
struct SCEVCollectStrides {
  ...
  bool follow(const SCEV *S) {
    if (const SCEVAddRecExpr *AR = dyn_cast<SCEVAddRecExpr>(S)) {
      ...
      if (auto *ME = dyn_cast<SCEVMulExpr>(AR->getStart()))
        Strides.push_back(ME);
    }
    return true;
  }
...
void ScalarEvolution::collectParametricTerms(const SCEV *Expr,
  ...
  for (const SCEV *S : Strides) {
    if (SCEVExprContains(S, isa<SCEVUnknown, const SCEV *>))
      Terms.push_back(S);
  }
...
static bool findArrayDimensionsRec(ScalarEvolution &SE, const SCEV *ES,
                                   SmallVectorImpl<const SCEV *> &Terms,
                                   SmallVectorImpl<const SCEV *> &Sizes) {
  int Last = Terms.size() - 1;
  const SCEV *Step = Terms[Last];
  DEBUG({
    errs() << "\nfAD Step: " << *Step << " Last: " << Last << "\n";
    for (auto *T : Terms)
      errs() << "\t term: " << *T << "\n";
  });
  ...
  if (Last == 0) {
    ...
    DEBUG(errs() << "\n Found last size: " << *Step << "\n");
    Sizes.push_back(Step);
    return true;
  }

  SmallVector<const SCEV *, 8> NewTerms;
  for (unsigned u = 0; u < Terms.size(); u++) {
    const SCEV *Term = Terms[u];

    // Normalize the terms before the next call to findArrayDimensionsRec.
    const SCEV *Q, *R;
    SCEVDivision::divide(SE, Term, Step, &Q, &R);
    DEBUG(errs() << "TD: " << u << " T: " << *Term << " / " << *Step << " = "
                 << *Q << "  R:" << *R << "\n");

    if (!R->isZero()) {
      if (Q->isZero()) {
        auto *NewStep = removeConstantFactors(SE, Step);
        if (!NewStep || NewStep == Step)
          return false;
        DEBUG(errs() << "Step: " << *Step << "  NewStep: " << *NewStep << "\n");
        Terms.push_back(NewStep);
        return findArrayDimensionsRec(SE, ES, Terms, Sizes);
      }

#if 0
      const SCEV *Factor, *Rem;
      SCEVDivision::divide(SE, R, Q, &Factor, &Rem);
      if (!Rem->isZero())
        return false;
      DEBUG(errs() << "F0 " << *Term << " / " << *Step << " = " << *Q << " + " << *Q
             << " * " << *Factor << "\n");
      Terms[Last] = Step = SE.getAddExpr(Step, Factor);
      SCEVDivision::divide(SE, Term, Step, &Q, &R);
      DEBUG(errs() << "F1 " << *Term << " / " << *Step << " = " << *Q << "  R: " << *R
             << "\n");
      assert(R->isZero());
#endif
    }

    NewTerms.push_back(Q);
  }
  ...
  if (Terms.size() > 0)
    if (!findArrayDimensionsRec(SE, ES, Terms, Sizes))
      return false;
...
struct SCEVTermCounter {
  ScalarEvolution &SE;
  int Sum;

  SCEVTermCounter(ScalarEvolution &SE) : SE(SE), Sum(0) {}

  bool follow(const SCEV *S) {
    if (const SCEVMulExpr *Mul = dyn_cast<SCEVMulExpr>(S))
      Sum += Mul->getNumOperands();
    return true;
  }
  bool isDone() const { return false; }
};
...
static inline int numberOfTerms(ScalarEvolution &SE, const SCEV *S) {
  DenseMap<const llvm::SCEVUnknown *, const llvm::SCEVConstant *> CounterMap;
  SCEVUnkownCounter::count(S, SE, CounterMap);
  if (CounterMap.empty())
    return 0;
  auto *SC = CounterMap.begin()->second;
  return SC->getValue()->getSExtValue() << CounterMap.size();
}

static const SCEV *removeConstantFactors(ScalarEvolution &SE, const SCEV *T, const SCEVConstant **F) {
  ...
  if (auto *SExt = dyn_cast<SCEVSignExtendExpr>(T)) {
    auto *NewT = removeConstantFactors(SE, SExt->getOperand(), F);
    if (F && *F)
      *F = cast<SCEVConstant>(SE.getSignExtendExpr(*F, T->getType()));
    return SE.getSignExtendExpr(NewT, T->getType());
  }

  if (const SCEVAddExpr *A = dyn_cast<SCEVAddExpr>(T)) {
    if (A->getNumOperands() != 2)
      return T;
    if (!isa<SCEVConstant>(A->getOperand(0)) && !isa<SCEVConstant>(A->getOperand(1)))
      return T;
    int IdxA = isa<SCEVConstant>(A->getOperand(0)) ? 1 : 0;
    const SCEVMulExpr *M = dyn_cast<SCEVMulExpr>(A->getOperand(IdxA));
    if (!M)
      return T;
    if (M->getNumOperands() != 2)
      return T;
    if (!isa<SCEVConstant>(M->getOperand(0)) && !isa<SCEVConstant>(M->getOperand(1)))
      return T;
    int IdxM = isa<SCEVConstant>(M->getOperand(0)) ? 1 : 0;
    const SCEV *Q, *R;
    SCEVDivision::divide(SE, A->getOperand(1 - IdxA), M->getOperand(1 - IdxM), &Q, &R);
    if (!R->isZero()) {
      SCEVDivision::divide(SE, M->getOperand(1 - IdxA), A->getOperand(1 - IdxM), &Q, &R);
      if (!R->isZero())
        return T;
      if (F)
        *F = cast<SCEVConstant>(A->getOperand(1 - IdxM));
      return SE.getAddExpr(SE.getMulExpr(M->getOperand(IdxM), Q), SE.getOne(T->getType()));
    } else {
      if (F)
        *F = cast<SCEVConstant>(M->getOperand(1 - IdxM));
      return SE.getAddExpr(M->getOperand(IdxM), Q);
    }
  }

  if (const SCEVMulExpr *M = dyn_cast<SCEVMulExpr>(T)) {
    SmallVector<const SCEV *, 2> Factors;
    const SCEV *C = SE.getOne(T->getType());
    for (const SCEV *Op : M->operands()) {
      if (auto *COp = dyn_cast<SCEVConstant>(Op)) {
        C = SE.getMulExpr(C, COp);
        continue;
      }

      const SCEVConstant *F = nullptr;
      Op = removeConstantFactors(SE, Op, &F);
      if (F)
        C = SE.getMulExpr(C, F);
...
void ScalarEvolution::findArrayDimensions(SmallVectorImpl<const SCEV *> &Terms,
  ...
  //SmallVector<const SCEV *, 8> NewTerms;
  SmallVector<const SCEVConstant *, 8> Factors;
  for (const SCEV *T : Terms) {
    const SCEVConstant *Factor = nullptr;
    auto *NewT = removeConstantFactors(SE, T, &Factor);
    if (NewT == T || !Factor || !isa<SCEVConstant>(Factor))
      break;
    DEBUG(errs() << "Factor: " << *Factor << " for " << *T << "\n");
    Factors.push_back(cast<SCEVConstant>(Factor));
    //NewTerms.push_back(NewT);
  }

  // Put larger terms first.
  std::sort(Factors.begin(), Factors.end(), [&](const SCEVConstant *LHS, const SCEVConstant *RHS) {
      return LHS->getValue()->getSExtValue() < RHS->getValue()->getSExtValue();
  });

  if (Factors.size() == Terms.size()) {
  ...
  for (const SCEV *T : Terms) {
    if (!T->getType()->isIntegerTy())
      return;
    MaxTyS = std::max(MaxTyS, SE.getTypeSizeInBits(T->getType()));
  }
  ...
  if (ElementSize->getType() != Ty)
    ElementSize = SE.getTruncateOrSignExtend(ElementSize, Ty);
  ...
  std::sort(Terms.begin(), Terms.end(), [&](const SCEV *LHS, const SCEV *RHS) {
    DenseMap<const llvm::SCEVUnknown *, const llvm::SCEVConstant *>
        CounterMapLHS, CounterMapRHS;
    SCEVUnkownCounter::count(LHS, SE, CounterMapLHS);
    SCEVUnkownCounter::count(RHS, SE, CounterMapRHS);
    if (CounterMapLHS.size() < CounterMapRHS.size())
      return false;
    if (CounterMapLHS.size() > CounterMapRHS.size())
      return true;
    for (auto &It : CounterMapLHS) {
      auto *LHSC = It.second;
      auto *RHSC = CounterMapRHS.lookup(It.first);
      if (!RHSC)
        return true;
      auto LHSV = LHSC->getValue()->getSExtValue();
      auto RHSV = RHSC->getValue()->getSExtValue();
      if (LHSV < RHSV)
        return false;
      if (LHSV > RHSV)
        return true;
  ...
  if (Terms.empty() || !findArrayDimensionsRec(SE, ElementSize, Terms, Sizes)) {
    Sizes.clear();
    return;
  }
  ...
  bool ContainsElementSize = false;
  for (const SCEV *&Size : Sizes) {
    const SCEV *Q, *R;
    SCEVDivision::divide(SE, Size, ElementSize, &Q, &R);
    if (!Q->isZero() && R->isZero()) {
      if (Q->isOne())
        ContainsElementSize = true;
      else
        Size = Q;
      //Q = removeConstantFactors(SE, Size);
      //if (Q && Q != Size)
        //Size = Q;
    }
  }
