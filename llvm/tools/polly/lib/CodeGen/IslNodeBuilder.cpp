  ...
  for (auto &SAIIt : MALoopMap) {
    ...
    for (auto &It0 : AccessMap) {
      ...
      if (!LastItMap) {
        ...
        unsigned InnerDim = NumSchedDims - 1;
        do {
          auto *Val =
        ...
        for (unsigned Dim = 0; Dim  < NumSchedDims; Dim++)
          if (Dim != InnerDim)
            LastItMap = isl_map_equate(LastItMap, isl_dim_in, Dim, isl_dim_out, Dim);
        //errs() << "GTM: " << LastItMap << " for " << Stmt->getBaseName() << "\n";
      ...
      if (LastDim >= 0 && !isl_map_involves_dims(AccRel0, isl_dim_in, LastDim, 1)) {
        isl_map_free(AccRel0);
        continue;
      }
      if (!isl_map_is_injective(AccRel0)) {
