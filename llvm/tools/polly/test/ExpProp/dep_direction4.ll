...
; CHECK-NEXT:            Domain :=
; CHECK-NEXT:                { Stmt_for_body_split[50] }; CopyDom: { Stmt_for_body_split[i0] : 0 <= i0 <= 99 };
; CHECK-NEXT:            Schedule :=
; CHECK-NEXT:                { Stmt_for_body_split[i0] -> [50, 0] };
; CHECK-NEXT:            MustWriteAccess :=	[Reduction Type: NONE] [Scalar: 0]
; CHECK-NEXT:                { Stmt_for_body_split[i0] -> MemRef_B[i0] };
; CHECK-NEXT:    	Stmt_for_body_split_split
; CHECK-NEXT:            Domain :=
; CHECK-NEXT:                { Stmt_for_body_split_split[99]; Stmt_for_body_split_split[49] };
; CHECK-NEXT:            Schedule :=
; CHECK-NEXT:                { Stmt_for_body_split_split[99] -> [99, 1]; Stmt_for_body_split_split[49] -> [49, 1] };
; CHECK-NEXT:            MustWriteAccess :=	[Reduction Type: NONE] [Scalar: 0]
; CHECK-NEXT:                { Stmt_for_body_split_split[i0] -> MemRef_A[50] };
; CHECK-NEXT:    	Stmt_for_body_split_0	 [#Recompute Values: 1]
; CHECK-NEXT:            Domain :=
; CHECK-NEXT:                { Stmt_for_body_split_0[i0] : 0 <= i0 <= 99 and (i0 >= 51 or i0 <= 49) };
; CHECK-NEXT:            Schedule :=
; CHECK-NEXT:                { Stmt_for_body_split_0[i0] -> [i0, 0] : i0 >= 51 or i0 <= 49 };
; CHECK-NEXT:            MustWriteAccess :=	[Reduction Type: NONE] [Scalar: 0]
...
; CHECK-NEXT:    }
;
source_filename = "test/ExpProp/dep_direction3.c"
