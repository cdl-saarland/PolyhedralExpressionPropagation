...
; RUN: opt %loadPolly -polly-exp -polly-exp-heuristic=0 -polly-opt-isl -polly-ast -analyze < %s | FileCheck %s --check-prefix=ISL_AST
;
; XFAIL: *
;
;    #include <math.h>
...
;
; AST:        if (N >= 1) {
; AST-NEXT:      if (N >= 2 && numSteps >= 2) {
; AST-NEXT:        Stmt_if_then116_0_0_0(0);
; AST-NEXT:      } else if (N == 1 && numSteps >= 2) {
; AST-NEXT:        Stmt_if_then116_0(0);
; AST-NEXT:      } else if (N == 1) {
; AST-NEXT:        Stmt_if_then116(0);
; AST-NEXT:      } else {
; AST-NEXT:        Stmt_if_then116_0_0(0);
; AST-NEXT:      }
; AST-NEXT:    }
;
